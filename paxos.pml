...
chan coorproposal[COORDINATORS] = [20] of {byte,byte}        /*所有的proposer都先将proposal发送到该通道给coordinator*/
...
chan promise[COORDINATORS] = [20] of {byte, byte,byte,byte} ;           /*coordinator向acceptor加入round的请求后用来接收promise信息*/
chan learnacc = [20] of {byte,byte,byte};                     /*acceptor接受一个value后发送到该通道用来提醒learner*/
...
proctype learner(){
    ...
    ::countacc >= MAJORITY -> 
             atomic{
             	learned ! crnd,cval  -> break;          /*progress*/  //若同一value计数超过majority，则被learn
             	}    
    //::progress == true ->break;                  
    od
}
